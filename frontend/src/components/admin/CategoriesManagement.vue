<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-4xl font-bold text-gray-900 flex items-center">
              <svg class="h-10 w-10 text-indigo-600 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Add Category Form -->
      <div v-if="showForm" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8">
      ...
      <!-- Loading State -->
      <div v-if="loading && !categories.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-12">
      ...
      <!-- Categories Table -->
      <div v-else-if="paginatedCategories.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <!-- Table Header -->
            ...
            <div class="flex items-center space-x-4">
              ...
              <div class="flex items-center space-x-2">
                <button
                  ...
                  v-if="selectedCategories.length < paginatedCategories.length"
                >
                  Select All
                </button>
                <button
          ...
          <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr>
                ...
                >
                  <div class="flex items-center space-x-1">
                    ...
                    <svg v-if="sortField === 'name'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center space-x-1">
                    <span>Stock ID</span>
                    <svg v-if="sortField === 'stock_id'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </div>
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr 
                ...
                :class="{ 'bg-indigo-50': selectedCategories.includes(category.id || index) }"
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center mr-3">
                      <span class="text-indigo-600 font-semibold text-sm">{{ category.id || index + 1 }}</span>
                    </div>
                    <span class="text-sm font-medium text-gray-900">#{{ category.id || index + 1 }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
      ...
      <div v-if="isShowing" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <h2 class="text-xl font-bold text-gray-900">Category Details</h2>
              <button 
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div class="flex justify-between items-center py-2 border-b border-gray-100">
                ...
                <span class="text-gray-900">{{ selectedCategory.stock_id }}</span>
              </div>
...
const endIndex = computed(() => {
  return Math.min(startIndex.value + itemsPerPage.value, filteredCategories.value.length)
})
...
const handleSubmit = async () => {
  try {
    ...
    const url = formMode.value === 'add' ? BASE_URL : `${BASE_URL}/${currentCategory.value.id}`
...
const deleteCategory = async (id) => {
  if (confirm('Are you sure you want to delete this category?')) {
    try {
      ...
      selectedCategories.value = selectedCategories.value.filter(selectedId => selectedId !== id)
