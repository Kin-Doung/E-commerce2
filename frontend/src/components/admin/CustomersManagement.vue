          ...
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-4xl font-bold text-gray-900 flex items-center">
                <svg
                  class="h-10 w-10 text-emerald-600 mr-4"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <div class="flex items-center space-x-4">
              ...
              <button
                @click="toggleAddForm"
                class="px-6 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors duration-200 flex items-center font-medium shadow-sm"
              >
                <svg
                  v-if="!showAddForm"
                  class="h-5 w-5 mr-2"
        ...
        >
          ...
          <form @submit.prevent="addCustomer" class="space-y-6">
            ...
            >
              ...
              <button
                type="submit"
                class="px-6 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                :disabled="loading"
              >
                <span
                  v-if="loading"
          ...
          >
            ...
            >
              ...
              <div class="relative">
                ...
                <input
                  ...
                  class="pl-10 pr-4 py-3 w-80 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                />
              </div>
            ...
            <div class="flex items-center space-x-4">
              ...
              <button
                ...
                class="p-3 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors"
                :disabled="loading"
              >
                <svg
                  class="h-5 w-5"
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr
                  ...
                  :class="{
                    'bg-emerald-50': selectedCustomers.includes(customer.id),
                  }"
                >
                  <!-- Checkbox -->
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div
                      class="h-10 w-10 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full flex items-center justify-center"
                    >
                  ...
                  >
                    <div class="relative inline-block text-left">
                      <button
                        ...
                        class="inline-flex items-center justify-center w-8 h-8 bg-white border border-gray-300 rounded-full shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-colors"
                        :class="{
                          'bg-gray-50 ring-2 ring-emerald-500':
                      ...
                      <div
                        ...
                        class="absolute right-0 z-20 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1"
                      >
                        <!-- View Option -->
                        <button
          ...
          <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <div class="p-6 space-y-4">
              <div class="flex items-center justify-center mb-6">
                <div
                  class="h-16 w-16 bg-emerald-100 rounded-full flex items-center justify-center"
                >
...
const selectedCustomer = ref({ id: null, name: "", email: "" });
...
const fetchCustomers = async () => {
  try {
    ...
    customers.value = data.map(({ id, name, email }) => ({ id, name, email }));
  } catch (err) {
...
const saveCustomer = async () => {
  const index = customers.value.findIndex(
    (c) => c.id === editedCustomer.value.id
  );
  if (index !== -1) {
    try {
      loading.value = true;
  ...
  if (
    ...
    try {
      ...
      customers.value = customers.value.filter(
        (c) => !selectedCustomers.value.includes(c.id)
      );
      selectedCustomers.value = [];
      showToastMessage(
        `${deletePromises.length} customers deleted successfully!`
      );
    } catch (err) {
...
const toggleSelectAll = () => {
  if (isAllSelected.value) {
    selectedCustomers.value = selectedCustomers.value.filter(
      (id) => !paginatedCustomers.value.some((customer) => customer.id === id)
    );
  } else {
    const currentPageIds = paginatedCustomers.value.map(
      (customer) => customer.id
    );
    selectedCustomers.value = [
      ...new Set([...selectedCustomers.value, ...currentPageIds]),
    ];
  }
};
