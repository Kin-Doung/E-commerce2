<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="container">
      <aside></aside>
      <div class="analytics max-w-7xl mx-auto">
        <!-- Error Message -->
        <div
          v-if="error"
          class="mb-6 p-4 bg-red-100 text-red-700 rounded-lg flex items-center"
        >
          <svg
            class="w-5 h-5 mr-2"
            fill="none"
          ...
          <div>
            ...
            <button
              @click="refreshData"
              class="text-red-800 hover:text-red-900 text-sm underline mt-1"
            >
              Try Again
            </button>
          </div>
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="mt-3 text-lg text-gray-600">
                View detailed analytics for AquaPure. Track sales, customer
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="mt-2 text-3xl font-bold text-gray-900">
                  ${{ formatNumber(analytics.salesThisMonth) }}
                </p>
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="mt-2 text-2xl font-bold text-gray-900">
                  {{ analytics.topProduct.name || "No data" }}
                </p>
                <div class="mt-2 flex items-center">
                  <span class="text-sm font-medium text-yellow-600"
                    >{{ analytics.topProduct.sales }} units sold</span
                  >
                </div>
              </div>
        ...
        >
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div
              ...
              class="h-64 flex items-end justify-between space-x-1"
            >
              <div
            ...
            >
              <span
                >{{
                  chartPeriods.find((p) => p.value === selectedChartPeriod)
                    ?.label
                }}
                ago</span
              >
          ...
          <div v-if="recentActivities.length > 0" class="space-y-4">
            ...
            >
              ...
              >
                ...
                <svg
                  v-else-if="activity.type === 'customer'"
...
const recentActivities = computed(() => {
  ...
  // Add recent orders
  orders.value.slice(0, 10).forEach((order) => {
    const user = users.value.find((u) => u.id === order.user_id);
    activities.push({
      id: `order-${order.id}`,
      type: "order",
      title: "New Order Received",
      description: `Order #${order.id} from ${
  ...
  users.value.slice(0, 5).forEach((user) => {
    activities.push({
      id: `user-${user.id}`,
      type: "customer",
...
const calculateAnalytics = () => {
  ...
  analytics.value.salesThisMonth = currentMonthOrders.reduce(
  ...
  // Calculate growth
  analytics.value.salesGrowth =
    lastMonthSales > 0
      ? ((analytics.value.salesThisMonth - lastMonthSales) / lastMonthSales) *
        100
      : 0;

  // Calculate total revenue
  analytics.value.totalRevenue = orders.value.reduce(
    (sum, order) => sum + parseFloat(order.total || 0),
    0
  );
...
const fetchChartData = async () => {
  ...
  orders.value.forEach((order) => {
    ...
    if (dateMap.has(orderDate)) {
      dateMap.set(
        orderDate,
        dateMap.get(orderDate) + parseFloat(order.total || 0)
      );
    }
  });
