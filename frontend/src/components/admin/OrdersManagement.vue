<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <!-- Loading State -->
      <div v-if="loading && !orders.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-12">
      ...
      <!-- Orders Table -->
      <div v-else-if="paginatedOrders.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <!-- Table Header -->
          ...
          <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr>
                ...
                >
                  <div class="flex items-center space-x-1">
                    <span>Order ID</span>
                    <svg v-if="sortField === 'id'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                >
                  <div class="flex items-center space-x-1">
                    <span>Customer</span>
                    <svg v-if="sortField === 'user.name'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center space-x-1">
                    <span>Total</span>
                    <svg v-if="sortField === 'total'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center space-x-1">
                    <span>Date</span>
                    <svg v-if="sortField === 'updated_at'" class="h-4 w-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </div>
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr 
                ...
                :class="{ 'bg-purple-50': selectedOrders.includes(order.id || index) }"
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center mr-3">
                      <span class="text-purple-600 font-semibold text-sm">#{{ order.id || index + 1 }}</span>
                    </div>
                    <span class="text-sm font-medium text-gray-900">Order #{{ order.id || index + 1 }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-10 h-10 bg-gradient-to-r from-purple-500 to-pink-600 rounded-full flex items-center justify-center mr-3">
                      <span class="text-white font-semibold text-sm">{{ getInitials(order.user?.name || 'Unknown') }}</span>
                    </div>
                    <div>
                      <div class="text-sm font-semibold text-gray-900">{{ order.user?.name || 'Unknown Customer' }}</div>
                      <div class="text-xs text-gray-500">{{ order.user?.email || 'No email' }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-lg font-bold text-green-600">${{ parseFloat(order.total || 0).toFixed(2) }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="getStatusClass(order.status || 'pending')">
                    <div class="w-1.5 h-1.5 rounded-full mr-1.5" :class="getStatusDotClass(order.status || 'pending')"></div>
                    {{ formatStatus(order.status || 'pending') }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{ formatDate(order.updated_at || order.created_at || new Date()) }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
                  <div class="relative inline-block">
                    <button
                      ...
                      :class="{ 'bg-gray-100 text-gray-600': activeMenuIndex === index }"
                    >
      ...
      <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 p-12">
        <div class="text-center">
          ...
          <p class="text-gray-600 mb-6">No orders match your current search criteria.</p>
        </div>
      </div>
          ...
          <div class="p-6 space-y-6">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span :class="getStatusClass(selectedOrder.status || 'pending')" class="ml-2">
                  {{ formatStatus(selectedOrder.status || 'pending') }}
                </span>
              </div>
              <div>
                <span class="font-medium text-gray-700">Customer:</span>
                <p class="text-gray-900">{{ selectedOrder.user?.name || 'Unknown' }}</p>
              </div>
              <div>
                <span class="font-medium text-gray-700">Email:</span>
                <p class="text-gray-900">{{ selectedOrder.user?.email || 'No email' }}</p>
              </div>
              <div>
                <span class="font-medium text-gray-700">Total:</span>
                <p class="text-2xl font-bold text-green-600">${{ parseFloat(selectedOrder.total || 0).toFixed(2) }}</p>
              </div>
              <div>
                <span class="font-medium text-gray-700">Date:</span>
                <p class="text-gray-900">{{ formatDate(selectedOrder.updated_at || selectedOrder.created_at) }}</p>
              </div>
            </div>
...
const filteredOrders = computed(() => {
  ...
  if (statusFilter.value) {
    filtered = filtered.filter(order => 
      (order.status || 'pending').toLowerCase() === statusFilter.value.toLowerCase()
    )
  }
...
const endIndex = computed(() => {
  return Math.min(startIndex.value + itemsPerPage.value, filteredOrders.value.length)
})
...
const updateOrderStatus = (order) => {
  ...
  showToastMessage(`Status update for order #${order.id} - Feature coming soon!`)
  activeMenuIndex.value = null
}
...
const deleteOrder = async (id) => {
  if (confirm('Are you sure you want to delete this order?')) {
    try {
      ...
      selectedOrders.value = selectedOrders.value.filter(selectedId => selectedId !== id)
...
const getStatusClass = (status) => {
  const classes = {
    pending: 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800',
    processing: 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800',
    shipped: 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-800',
    delivered: 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800',
    cancelled: 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800'
  }
