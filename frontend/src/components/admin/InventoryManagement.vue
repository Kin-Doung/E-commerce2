<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="container">
      ...
      <div class="inventory max-w-7xl mx-auto">
        <!-- Header Section -->
        <div
        ...
        <!-- View Details Modal -->
        <div
          v-if="showDetailModal && selectedInventory"
          class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"
        >
          <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="p-6 border-b border-gray-200">
            ...
            <div class="p-6 space-y-4">
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <span class="text-gray-900">{{
                    selectedInventory.product?.name || "N/A"
                  }}</span>
                </div>
                <div
                  class="flex justify-between items-center py-2 border-b border-gray-100"
...
const lowStockCount = computed(() => {
  return inventoryItems.value.filter((item) => parseInt(item.quantity) < 10)
    .length;
});
...
const handleSubmit = async () => {
  try {
    ...
    const url = isEditing.value
      ? `${BASE_URL}/${currentInventory.value.id}`
      : BASE_URL;
    ...
    if (!response.ok)
      throw new Error(
        `Failed to ${isEditing.value ? "update" : "add"} inventory item`
      );
    const data = await response.json();
