<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-lg text-gray-600 mt-2">
              Welcome to the Admin Dashboard. Monitor key metrics and manage your business operations.
            </p>
          </div>
          <div class="flex items-center space-x-4">
            ...
            >
              ...
              Cart
              <span v-if="allCarts.length > 0" class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                {{ allCarts.length }}
              </span>
            </button>
      ...
      <!-- Error State -->
      <div v-if="error && !dataLoaded" class="bg-red-50 border border-red-200 rounded-lg p-6 mb-8">
        <div class="flex items-center">
          <svg class="w-6 h-6 text-red-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
          </svg>
          <div>
            <h3 class="text-red-800 font-medium">Failed to load dashboard data</h3>
            <p class="text-red-600 text-sm mt-1">{{ error }}</p>
            <button @click="refreshDashboard" class="mt-2 text-red-600 hover:text-red-800 text-sm font-medium">
              Try Again
            </button>
          </div>
        </div>
      ...
      <!-- Key Metrics Grid -->
      <div v-if="dataLoaded" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <!-- Total Orders -->
        ...
        <!-- Active Carts -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow cursor-pointer" @click="showCartModal = true">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wide">Active Carts</h3>
              <p class="mt-2 text-3xl font-bold text-gray-900">{{ activeCarts.length }}</p>
              <div class="mt-2 flex items-center">
                <svg class="h-4 w-4 text-blue-500 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>
                </svg>
                <span class="text-sm font-medium text-blue-600">{{ abandonedCarts.length }} abandoned</span>
              </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div class="mt-4 space-y-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-sm font-medium text-gray-900">{{ activeCarts.length }}</span>
            </div>
            <div class="flex items-center justify-between">
              <div class="flex items-center">
              ...
              <span class="text-sm font-medium text-gray-900">{{ abandonedCarts.length }}</span>
            </div>
            <div class="flex items-center justify-between">
              <div class="flex items-center">
              ...
              <span class="text-sm font-medium text-gray-900">{{ metrics.totalOrders }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Recent Orders -->
      <div v-if="dataLoaded" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
      ...
      <!-- Cart Modal -->
      <div v-if="showCartModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
          <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Cart Management</h3>
            <button 
          ...
          <div class="p-6 overflow-y-auto max-h-[70vh]">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div class="bg-blue-50 p-4 rounded-lg">
                <h4 class="text-sm font-medium text-blue-900 mb-2">Active Carts</h4>
                <p class="text-2xl font-bold text-blue-900">{{ activeCarts.length }}</p>
              </div>
              <div class="bg-red-50 p-4 rounded-lg">
                <h4 class="text-sm font-medium text-red-900 mb-2">Abandoned Carts</h4>
                <p class="text-2xl font-bold text-red-900">{{ abandonedCarts.length }}</p>
              </div>
              <div class="bg-green-50 p-4 rounded-lg">
                <h4 class="text-sm font-medium text-green-900 mb-2">Total Value</h4>
                <p class="text-2xl font-bold text-green-900">${{ totalCartValue.toFixed(2) }}</p>
              </div>
            </div>
            
            <div class="space-y-4">
              <h4 class="text-lg font-semibold text-gray-900">All Carts</h4>
              <div v-if="allCarts.length === 0" class="text-center py-8 text-gray-500">
                <svg class="mx-auto w-12 h-12 text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.5 6M7 13l-1.5-6m0 0h15"/>
                </svg>
              ...
              <div v-else class="space-y-3">
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div>
                      <p class="text-sm font-medium text-gray-900">Cart #{{ cart.id }}</p>
                      <p class="text-sm text-gray-500">{{ cart.user_name || `User #${cart.user_id}` }}</p>
                      <p class="text-xs text-gray-400">{{ cart.items_count }} items • {{ formatTime(cart.updated_at) }}</p>
                    </div>
                  </div>
                  <div class="text-right">
                    <p class="text-sm font-medium text-gray-900">${{ cart.total_value.toFixed(2) }}</p>
                    <span :class="[
                      'text-xs px-2 py-1 rounded-full font-medium',
                      cart.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    ]">
                      {{ cart.status }}
                    </span>
...
const fetchCartData = async () => {
  try {
    ...
    allCarts.value = carts.map(cart => ({
      ...
      total_value: cart.cart_items?.reduce((sum, item) => sum + (item.quantity * item.price), 0) || 0,
      status: cart.status || 'active',
      updated_at: cart.updated_at,
      created_at: cart.created_at
    }))
    
    activeCarts.value = allCarts.value.filter(cart => cart.status === 'active')
    abandonedCarts.value = allCarts.value.filter(cart => cart.status === 'abandoned')
...
const calculateMetrics = () => {
  ...
  const uniqueCustomers = [...new Set(orders.value.map(order => order.user_id))]
  metrics.value.activeCustomers = uniqueCustomers.length
  
  // Calculate pending tasks (orders without completed status)
  metrics.value.pendingTasks = orders.value.filter(order => 
    !order.order_items?.some(item => item.status === 'completed')
  ).length
  ...
  recentOrders.value = orders.value
    ...
    .map(order => ({
      id: order.id,
      customer: users.value.find(u => u.id === order.user_id)?.name || 'Unknown Customer',
      product: products.value.find(p => p.id === order.order_items?.[0]?.product_id)?.name || 'Unknown Product',
  ...
  try {
    ...
    orders.value.forEach(order => {
      ...
      if (dateMap.has(orderDate)) {
        dateMap.set(orderDate, dateMap.get(orderDate) + Number(order.total || 0))
      }
    })
    
    chartData.value = {
      labels: Array.from(dateMap.keys()).reverse().map(date => {
